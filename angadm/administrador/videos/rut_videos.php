	...
	<section>
		...
		<article id="automargen">
			<article class="columninput">
				<label>Del Titulo video</label>
				<select id="idVd">
					<option value="0">Selecione</option>
					<?php
						$vidT="SELECT * from videos order by nam_vid asc";
						$sql_vidT=mysql_query($vidT,$conexion) or die (mysql_error());
						while ($yv=mysql_fetch_array($sql_vidT)) {
							$idvt=$yv['id_video'];
							$nmvt=$yv['nam_vid'];
					?>
					<option value="<?php echo $idvt ?>"><?php echo "$nmvt"; ?></option>
					<?php
						}
					?>
				</select>
				...
				<div id="txD"></div>
				<input type="submit" value="Ingresar" id="rutrlyb" class="botonstyle" />
			</article>
		</article>
	</section>
	<footer>
