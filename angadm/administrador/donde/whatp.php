		...
		<article id="automargen">
			<article id="mD" class="ocultingre">
				<article class="columninput">
					...
					<input type="text" id="mpbd" placeholder="https://www.google.com/maps/d/embed?mid=zOTyyKuZ2RUI.kz5y3dwR3cyc" />
			...
			<article class="columninput">
				<label>De la bodega</label>
				<select id="rdbg">
					<option value="0">Seleccione</option>
					<?php
						$Tbd="SELECT * from bodega order by nam_bd asc";
						$sql_Tbd=mysql_query($Tbd,$conexion) or die (mysql_error());
						while ($tT=mysql_fetch_array($sql_Tbd)) {
							$idT=$tT['id_bodega'];
							$nmT=$tT['nam_bd'];
					?>
					<option value="<?php echo $idT ?>"><?php echo "$nmT"; ?></option>
					<?php
						}
					?>
				</select>
