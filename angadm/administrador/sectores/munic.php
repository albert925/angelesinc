...
<body>
	...
	<article id="mnuPp">
		<nav id="mnP">
			<ul>
				...
				<li><a class="selC" href="../sectores">Sectores</a></li>
	...
	<nav id="mnA">
		...
		<a href="../sectores">Departamentos</a>
		<a href="munic.php" class="seliA">Municipios</a>
	</nav>
	<section>
		<h1>Sectores</h1>
		<h2>Municipios</h2>
		<article id="automargen">
			...
			<?php
				...
				$ssql="SELECT * from municipio order by nam_muni asc";
				$rs=mysql_query($ssql,$conexion) or die (mysql_error());
				$num_total_registros= mysql_num_rows($rs);
				$total_paginas= ceil($num_total_registros / $tamno_pagina);
				$gsql="SELECT * from municipio order by nam_muni asc limit $inicio, $tamno_pagina";
				$impsql=mysql_query($gsql,$conexion) or die (mysql_error());
				while ($gh=mysql_fetch_array($impsql)) {
					$idmn=$gh['id_muni'];
					$nmmn=$gh['nam_muni'];
			?>
			<article id="cajas_al" class="columninput">
				<input type="text" id="nmFmn_<?php echo $idmn ?>" value="<?php echo $nmmn ?>" />
				<div id="txB_<?php echo $idmn ?>"></div>
				<input type="submit" value="Modificar" class="cammunic" id="botonB" data-id="<?php echo $idmn ?>" />
			</article>
			<?php
				}
			?>
		...
		<article>
			...
			?>
						<a href="munic.php?pagina=<?php echo $i ?>"><?php echo "$i"; ?></a>
