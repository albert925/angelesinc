...
<body>
	...
	<nav id="mnA">
		...
		<a href="img_p.php" class="seliA">Nueva Imagen Producto</a>
	</nav>
	<section>
		<h1>Nueva imagen de Productos</h1>
			...
			<article class="columninput">
				<form action="#" method="post" enctype="multipart/form-data" id="nvPd_image" class="columninput">
					...
					<select id="pB" name="pB">
						...
						<?php
							$gsql="SELECT * from producto order by cod_p desc";
							$impsql=mysql_query($gsql,$conexion) or die (mysql_error());
							while ($gh=mysql_fetch_array($impsql)) {
								$idPr=$gh['cod_p'];
								$nmPr=$gh['nam_p'];
						?>
						<option value="<?php echo $idPr ?>"><?php echo "$idPr--$nmPr"; ?></option>
						<?php
							}
						?>
					</select>
					...
					<div id="txH"></div>
					<input type="submit" value="Subir e Ingresar" id="imPsub" class="botonstyle" />
				</form>
			</article>
		</article>
	</section>
