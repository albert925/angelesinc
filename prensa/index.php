	...
	<section id="automargen">
		<article class="fleaxtDlx">
			<?php
				...
				$ssql="SELECT * from eventos order by id_evet desc";
				$rs=mysql_query($ssql,$conexion) or die (mysql_error());
				$num_total_registros= mysql_num_rows($rs);
				$total_paginas= ceil($num_total_registros / $tamno_pagina);
				$gsql="SELECT * from eventos order by id_evet desc limit $inicio, $tamno_pagina";
				...
				while ($gh=mysql_fetch_array($impsql)) {
					...
					$txtE=$gh['text_evet'];
					$ffE=$gh['fech_evet'];
					$primer_imgaen="SELECT * from iamges_evet 
						where evet_id=$idE order by id_img_evet asc limit 1";
					$sql_primer=mysql_query($primer_imgaen,$conexion) or die (mysql_error());
					while ($pgm=mysql_fetch_array($sql_primer)) {
						$idprmgm=$pgm['id_img_evet'];
						$rutimgE=$pgm['rut_evet'];
					}
			?>
			<figure id="vidL">
				<h2><?php echo "$nmE"; ?></h2>
				<a href="ind2x.php?ev=<?php echo $idE ?>">
					<img src="../<?php echo $rutimgE ?>" alt="<?php echo $nmE ?>" />
					<figcacption id="Eve">
						<?php echo "$txtE"; ?>
					</figcacption>
				</a>
