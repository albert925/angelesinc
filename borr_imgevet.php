	...
	if (isset($_SESSION['adm'])) {
		...
		if ($idR=="" || $cdP=="") {
			echo "<script type='text/javascript'>";
				echo "alert('Id de imagen o evento no disponible');";
				echo "var pagina='angadm/administrador/eventos';";
				echo "document.location.href=pagina;";
			echo "</script>";
		}
		else{
			$rutaB="SELECT * from iamges_evet where id_img_evet='$idR'";
			$sql_bri=mysql_query($rutaB,$conexion) or die (mysql_error());
			while ($tr=mysql_fetch_array($sql_bri)) {
				$rutaBor=$tr['rut_evet'];
			}
			unlink($rutaBor);
			$borrar="DELETE from iamges_evet where id_img_evet='$idR'";
			mysql_query($borrar,$conexion) or die (mysql_error());
			echo "<script type='text/javascript'>";
				echo "alert('imagen borrada');";
				echo "var pagina='angadm/administrador/eventos/images_eve.php?EV=$cdP';";
				echo "document.location.href=pagina;";
			echo "</script>";
		}
	}
